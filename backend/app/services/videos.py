...
class VideosService:
    def __init__(self, *, repo: VideosRepository):
        self._repo = repo
    ...
    @staticmethod
    def _edge_sentiment(summary_obj: Any) -> str:
    ...
    @classmethod
    def _summary_key_points(cls, summary_obj: Any, *, max_points: int = 10) -> list[str]:
        if not isinstance(summary_obj, dict):
            return []
    ...
    def list_videos(self, *, date_: date | None, days: int | None, limit: int) -> list[dict[str, Any]]:
        ...
        rows = self._repo.fetch_video_list_rows(start_iso=start_iso, end_iso=end_iso, limit=limit)
        ...
        for row in rows:
            ...
            if "title" in row:
                row["title"] = self._clean_text(row.get("title"))
            if "channel" in row:
                row["channel"] = self._clean_text(row.get("channel"))
    ...
    def video_infographic(self, *, date_: date | None, days: int, limit: int) -> list[dict[str, Any]]:
        ...
        videos = [r for r in self._repo.fetch_videos_basic_window(start_iso=start, end_iso=end, limit=limit) if r.get("video_id")]
        if not videos:
            return []

        video_ids = [str(v.get("video_id")) for v in videos if v.get("video_id")]
        summary_rows = self._repo.fetch_summaries_for_video_ids(video_ids=video_ids, limit=5000)
        ...
        for row in summary_rows:
            ...
            key_points = self._summary_key_points(summary_obj, max_points=10)
            sentiment = self._edge_sentiment(summary_obj)
            ...
            out.append(
                {
                    ...
                    "channel": self._clean_text(v.get("channel")),
    ...
    def get_video_detail(self, *, video_id: str) -> dict[str, Any] | None:
        video = self._repo.fetch_video_row(video_id=video_id)
        if not isinstance(video, dict) or not video:
            return None

        if "title" in video:
            video["title"] = self._clean_text(video.get("title"))
        if "channel" in video:
            video["channel"] = self._clean_text(video.get("channel"))
        if "id" not in video:
            video["id"] = video.get("video_id")

        vs_obj = self._repo.fetch_video_summary_row(video_id=video_id) or {}

        t_rows = self._repo.fetch_video_ticker_rows(video_id=video_id, limit=500)
        tickers: list[str] = []
        seen: set[str] = set()
        market_seen = False
        for r in t_rows:
        ...
        summary = {
            ...
            "video_titles": [
                self._clean_text(x) for x in (vs_obj.get("video_titles") or []) if str(x or "").strip()
            ]
        ...
        per_rows = self._repo.fetch_latest_per_ticker_summary_rows(video_id=video_id, limit=500)
            ...
            ticker_details.append(
                {
                    ...
                    "summary": summary_obj,
                    "sentiment": self._edge_sentiment(summary_obj),
                    "key_points": self._summary_key_points(summary_obj, max_points=12),
                }
            )
