...
class EntitiesService:
    ...
    def __init__(self, *, repo: EntitiesRepository):
        self._repo = repo
    ...
    @classmethod
    def _keypoints_by_sentiment_from_summary(
        cls,
        summary_obj: Any,
        *,
        ...
        max_items_per_bucket = cls._clamp_int(max_items_per_bucket, min_value=1, max_value=50)
    ...
    def top_movers(self, *, date_: date | None, days: int, limit: int) -> list[dict[str, Any]]:
        days = self._clamp_int(days, min_value=1, max_value=30)
        limit = self._clamp_int(limit, min_value=1, max_value=50)
        ...
        video_ids = self._repo.fetch_video_ids_in_window(
            start_iso=start,
            end_iso=end,
            limit=self._MAX_VIDEOS_SCAN,
        )
        if not video_ids:
            return []

        acc: dict[str, dict[str, Any]] = {}
        for group in self._chunked(video_ids, self._VIDEO_ID_CHUNK_SIZE):
            rows = self._repo.fetch_summaries_for_video_ids(video_ids=list(group), limit=self._SUMMARY_BATCH_LIMIT)
            for r in rows:
                ticker = r.get("ticker")
                if not ticker:
                    continue
                sym = self._normalize_symbol(str(ticker))
                if not sym or sym == "MARKET":
                    continue

                summary_obj = r.get("summary")
                p, n, u = self._summary_sentiment_counts(summary_obj)
                if p + n + u <= 0:
                    u = 1

                bucket = acc.setdefault(
                    sym,
                    {"symbol": sym, "positive": 0, "negative": 0, "neutral": 0, "reason": None},
                )
                ...
                if not bucket.get("reason"):
                    reason = self._first_claim_from_summary(summary_obj)
                    if reason:
    ...
    def chunks_for_entity(self, *, symbol: str, days: int, limit: int) -> list[dict[str, Any]]:
        sym = self._normalize_symbol(symbol)
        if not sym:
            return []

        days = self._clamp_int(days, min_value=1, max_value=30)
        limit = self._clamp_int(limit, min_value=1, max_value=500)
        ...
        vids = self._repo.fetch_recent_videos(start_iso=start, end_iso=end, limit=self._MAX_VIDEOS_SCAN)
        allowed_video_ids = [str(v.get("video_id")) for v in vids if isinstance(v, dict) and v.get("video_id")]
        video_meta: dict[str, dict[str, Any]] = {
            str(v.get("video_id")): v for v in vids if isinstance(v, dict) and v.get("video_id")
        }

        if not allowed_video_ids:
            return []

        prefetch = min(max(limit * self._CHUNKS_PREFETCH_MULTIPLIER, limit), self._CHUNKS_PREFETCH_CAP)
        ca_rows = self._repo.fetch_entity_summary_rows(symbol=sym, allowed_video_ids=allowed_video_ids, limit=prefetch)
        if not ca_rows:
            return []
        ...
        for r in ca_rows:
            ...
            summary_obj = r.get("summary")
            keypoints_by_sentiment = self._keypoints_by_sentiment_from_summary(summary_obj)
