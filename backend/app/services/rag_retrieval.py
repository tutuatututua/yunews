...
class RagRetrievalService:
    ...
    def retrieve_chunks(
        ...
        *,
        ...
        q_vec = self._embedder.embed(q_for_embedding)
        if not q_vec:
        ...
        def run_stage(*, stage: str, document_type: str | None, filter_ticker: str | None, match_count: int) -> None:
            method = " | ".join([stage, f"type={document_type or '*'}", f"ticker={filter_ticker or '*'}"])
            before = len(best_by_id)

            rows = self._repo.match_rag_documents(
        ...
        for c in final[:3]:
            ...
            top_bits.append(f"type={c.document_type} ticker={c.ticker or '-'} sim={c.similarity:.3f} video={title}".strip())
