...
from app.api.deps import get_entities_service
...
def top_movers(
	...
	svc: EntitiesService = Depends(get_entities_service),
) -> dict:
	...
	return {"data": svc.top_movers(date_=date_, days=days, limit=limit)}
...
def chunks_for_entity(
	...
	return {"data": svc.chunks_for_entity(symbol=symbol, days=days, limit=limit)}
